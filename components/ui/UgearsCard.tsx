...
export const UgearsCard: React.FC<UgearsCardProps> = ({
  variant = 'mechanical',
  children,
  className = '',
  style = {},
  ...
  const variantClasses = {
    mechanical: 'ugears-card-mechanical',
    wood: 'ugears-card-wood',
  ...
  const animationClass = animated ? 'ugears-transition-bounce' : '';
      ...
      {gearDecoration && (
        <>
          <div className="absolute top-2 right-2 opacity-20">
            <UgearsGear size="mini" color="brass" speed="slow" />
          </div>
          <div className="absolute bottom-2 left-2 opacity-20">
            <UgearsGear size="mini" color="steel" speed="reverse" />
          </div>
        </>
      )}

...
const VarkResultDisplay: React.FC<VarkResultDisplayProps> = ({ result, onContinue }) => {
  ...
  return (
    <UgearsCard variant="mechanical" className="p-8 text-center">
      <UgearsGear size="xl" color="brass" speed="slow" className="mx-auto mb-6" />
      <h2 className="text-3xl font-bold mb-4 ugears-heading-professional text-ugears-moonlight-blue">
        Your Learning Style Profile
      </h2>
      <p className="text-lg text-gray-700 mb-8 ugears-text-technical">
        Based on your responses, your primary learning preference is:
      </p>

      <div className="mb-8">
        <UgearsCard variant="wood" className="inline-block p-6">
          <div className="flex items-center justify-center space-x-4">
            <span className={`text-5xl ${primaryStyle.color}`}>{primaryStyle.icon}</span>
            <div>
              <h3 className="text-4xl font-bold ugears-heading-mechanical text-ugears-text-wood">
                {primaryStyle.label}
              </h3>
              <p className="text-gray-600 ugears-text-technical">{primaryStyle.description}</p>
            </div>
          </div>
        </UgearsCard>
      </div>
      ...
      <div className="mb-8 text-left">
        <h4 className="text-xl font-bold mb-4 ugears-heading-professional text-ugears-moonlight-blue">
          Your VARK Scores:
        </h4>
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          {Object.entries(vark_profile_data).map(([style, score]) => {
            ...
            return (
              <UgearsCard key={style} variant="mechanical" className="p-4 flex items-center space-x-4">
                <span className={`text-3xl ${details.color}`}>{details.icon}</span>
                <div>
                  <p className="font-semibold ugears-text-technical">{details.label}</p>
                  <p className="text-2xl font-bold ugears-heading-mechanical">{score}</p>
                </div>
              </UgearsCard>
            );
          })}
      ...
      {total_duration !== undefined && (
        <p className="text-sm text-gray-500 mb-8 ugears-text-technical">
      ...
      <div className="text-left mb-8 ugears-text-technical">
      ...
      <UgearsButton variant="mechanical" onClick={onContinue} className="w-full md:w-auto">

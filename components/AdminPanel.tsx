...
export default function AdminPanel() {
  ...
  useEffect(() => {
    ...
    if (isDemo) {
      setUsers([
        { uid: 'demo-user', email: 'demo@example.com', role: 'user', getIdToken: () => Promise.resolve('demo-token') },
        { uid: 'admin-user', email: 'admin@example.com', role: 'admin', getIdToken: () => Promise.resolve('admin-token') },
      ]);
      setLoading(false);
      return;
    }
  ...
  const handleRoleChange = async (uid: string, newRole: string) => {
    ...
    try {
      ...
      await axios.post(
        `${process.env.NEXT_PUBLIC_API_URL}/api/admin/role`,
        { uid, role: newRole },
        {
          headers: { Authorization: `Bearer ${token}` },
        }
